...
def create_optimized_marked_chunks(extracted_text: str):
    ...
    # Diagnostic logging for debugging chunking issues
    if not extracted_text:
        logging.warning("[CHUNKER] Extracted text is empty or None")
        return []
    
    logging.info(f"[CHUNKER] Received extracted text length: {len(extracted_text)} characters")
    
    # Check for expected markers
    has_doc_start = "DOCUMENT" in extracted_text and "CONTENTS STARTS HERE" in extracted_text
    has_page_markers = "PAGE NUMBER" in extracted_text and "STARTS HERE" in extracted_text
    logging.info(f"[CHUNKER] Marker check - Has DOCUMENT marker: {has_doc_start}, Has PAGE markers: {has_page_markers}")
    
    if not has_doc_start:
        logging.warning(f"[CHUNKER] Missing DOCUMENT markers. First 500 chars: {extracted_text[:500]}")
    ...
    logging.info(f"The Function create_optimized_marked_chunks completed in {processing_time_taken} seconds")
